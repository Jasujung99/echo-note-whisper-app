...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_vm_created_at 
  ON voice_messages(created_at DESC);

-- Compound index for sender queries with type filtering
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_vm_sender_type_created 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_vm_message_type 
  ON voice_messages(message_type);

-- 4. Add a partial index for broadcast messages (most common type)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_vm_broadcast_created 
  ON voice_messages(created_at DESC) 
